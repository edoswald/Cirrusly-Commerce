            ...
            if(res.success) {
                ...
                $.each(res.data, function(i, p){
                    ...
                    var row = '<tr>' +
                        ...
                        '<td><button type="button" class="cirrusly-btn-secondary cirrusly-edit-promo" style="padding: 6px 12px; border-radius: 6px; font-weight: 600; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); display: inline-flex; align-items: center; gap: 4px;" ' +
                        'data-id="'+ccEscapeHtml(p.id)+'" data-title="'+ccEscapeHtml(p.title)+'" data-dates="'+ccEscapeHtml(p.dates)+'" data-app="'+ccEscapeHtml(p.app)+'" data-type="'+ccEscapeHtml(p.type)+'" data-code="'+ccEscapeHtml(p.code || '')+'"><span class="dashicons dashicons-edit" style="line-height:inherit;"></span> Edit</button></td>' +
                        '</tr>';
                    $table.append(row);
                });
            } else {
