jQuery(document).ready(function($) {
    // Add toolbar-active class if Pricing Intelligence toolbar is present
    if ($('.cirrusly-pricing-engine-wrapper').length > 0) {
        document.body.classList.add('cirrusly-toolbar-active');
    }

    // Initialize datetime picker for Sale Timer End field (variations only)
    function initDateTimePicker() {
        if (typeof $.fn.datetimepicker !== 'undefined') {
            $('.cirrusly-datetime-picker:not(.hasDatepicker)').datetimepicker({
                dateFormat: 'yy-mm-dd',
                timeFormat: 'HH:mm',
                minDate: 0,
                stepMinute: 15
    ...
    // Re-initialize when WooCommerce adds new variation
    $(document).on('woocommerce_variations_loaded', initDateTimePicker);
    ...
    function getContext($el) {
        ...
        if($c.hasClass('woocommerce_variation')) return {
            ...
            // Updated Selectors
            display: $c.find('.cirrusly-metrics-display'),
            matrix: $c.find('.cirrusly-shipping-matrix'),
            rounding: $c.find('.cirrusly-sale-rounding')
        };
        else {
            // For simple products, ensure we can find WooCommerce native fields even from within the pricing toolbar
            var $regPrice = $c.find('#_regular_price');
            if (!$regPrice.length) $regPrice = $('#_regular_price'); // Fallback to document-level search

            var $salePrice = $c.find('#_sale_price');
            if (!$salePrice.length) $salePrice = $('#_sale_price'); // Fallback to document-level search

            // Add fallbacks for pricing fields for consistency and future-proofing
            var $cost = $c.find('#_cogs_total_value');
            if (!$cost.length) $cost = $('#_cogs_total_value');

            var $min = $c.find('#_auto_pricing_min_price');
            if (!$min.length) $min = $('#_auto_pricing_min_price');

            var $msrp = $c.find('#_alg_msrp');
            if (!$msrp.length) $msrp = $('#_alg_msrp');

            var $ship = $c.find('#_cirrusly_est_shipping');
            if (!$ship.length) $ship = $('#_cirrusly_est_shipping');

            var $shipClass = $c.find('#product_shipping_class');
            if (!$shipClass.length) $shipClass = $('#product_shipping_class');

            return {
                reg: $regPrice.length ? $regPrice : $c.find('#_subscription_price'),
                sale: $salePrice.length ? $salePrice : $c.find('#_subscription_sale_price'),
                cost: $cost,
                min: $min,
                msrp: $msrp,
                ship: $ship,
                shipClass: $shipClass,
                display: $('.cirrusly-metrics-display'),
    ...
    function updateMetrics($el) {
        ...
        if (price > 0 && total_cost > 0) {
            ...
            // Update main metrics display (new card-based UI)
            ctx.display.find('.cirrusly-profit-val').text('$'+net.toFixed(2));
            ctx.display.find('.cirrusly-margin-val').html(margin.toFixed(1)+'%');
            ctx.display.find('.cirrusly-min-price-val').text(min > 0 ? '$'+min.toFixed(2) : '--');
            
            // Build shipping scenario matrix
            if(ctx.matrix.length) {
                ctx.matrix.empty(); // Clear existing scenarios before rebuilding
                var rules = cirrusly_ship_config['matrix_rules'];
                var rulesObj = Array.isArray(rules) ? rules : Object.values(rules);
                
                if(rulesObj.length) {
                    $.each(rulesObj, function(k,v){
                        // Calculate scenario: price + shipping revenue - cost - fees - (shipping cost * multiplier)
                        var scenarioShipCost = ship * v.cost_mult;
                        var scenarioShipRev = getShippingRevenue(price); // Revenue stays same
                        var scenarioTotalInc = price + scenarioShipRev;
                        var scenarioFee = getFee(scenarioTotalInc);
                        var scenarioNet = scenarioTotalInc - cost - scenarioShipCost - scenarioFee;
                        var scenarioMargin = price > 0 ? (scenarioNet / price) * 100 : 0;
                        
                        // Determine profitability status
                        var profitClass = '';
                        var marginClass = '';
                        if (scenarioNet < 0) {
                            profitClass = 'cirrusly-profit-negative';
                            marginClass = 'cirrusly-profit-negative';
                        } else if (scenarioMargin < 10) {
                            profitClass = 'cirrusly-profit-warning';
                            marginClass = 'cirrusly-profit-warning';
                        } else {
                            profitClass = 'cirrusly-profit-positive';
                            marginClass = 'cirrusly-profit-positive';
                        }
                        
                        // Create scenario row element with proper DOM construction (XSS prevention)
                        var scenarioRow = document.createElement('div');
                        scenarioRow.className = 'cirrusly-scenario-row';
                        
                        // Label div - use textContent to prevent XSS
                        var labelDiv = document.createElement('div');
                        labelDiv.className = 'cirrusly-scenario-label';
                        labelDiv.textContent = v.label;
                        scenarioRow.appendChild(labelDiv);
                        
                        // Multiplier div - use textContent with formatted string
                        var multDiv = document.createElement('div');
                        multDiv.className = 'cirrusly-scenario-mult';
                        multDiv.textContent = 'x' + v.cost_mult + ' base cost';
                        scenarioRow.appendChild(multDiv);
                        
                        // Profit div - use textContent with formatted number
                        var profitDiv = document.createElement('div');
                        profitDiv.className = 'cirrusly-scenario-profit ' + profitClass;
                        profitDiv.textContent = '$' + scenarioNet.toFixed(2);
                        scenarioRow.appendChild(profitDiv);
                        
                        // Margin div - use textContent with formatted number
                        var marginDiv = document.createElement('div');
                        marginDiv.className = 'cirrusly-scenario-margin ' + marginClass;
                        marginDiv.textContent = scenarioMargin.toFixed(1) + '%';
                        scenarioRow.appendChild(marginDiv);
                        
                        // Append constructed row to container
                        ctx.matrix[0].appendChild(scenarioRow);
                    });
                } else {
                    // No scenarios configured - show helpful message (safe static content)
                    var messageRow = document.createElement('div');
                    messageRow.className = 'cirrusly-scenario-row';
                    messageRow.style.gridTemplateColumns = '1fr';
                    messageRow.style.justifyItems = 'center';
                    messageRow.style.color = '#646970';
                    messageRow.style.fontStyle = 'italic';
                    messageRow.textContent = 'No shipping scenarios configured. Add scenarios in Settings → Profit Engine.';
                    ctx.matrix[0].appendChild(messageRow);
                }
            }
        }
    }
