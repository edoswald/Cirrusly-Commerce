...
class Cirrusly_Commerce_GMC {
    ...
    public static function get_monitored_terms() {
        return array(
            ...
            'medical' => array( 
                'cure'        => array('severity' => 'Critical', 'scope' => 'all', 'reason' => 'Medical claim (Prohibited).', 'case_sensitive' => true),
                'heal'        => array('severity' => 'Critical', 'scope' => 'all', 'reason' => 'Medical claim implying permanent fix.', 'case_sensitive' => true),
     ...
     */
    /**
     * Regex pattern for punctuation removal in normalization.
     */
    private static $punct_pattern = '/[.!?,;:()\[\]]+/';
    ...
    private static function get_issue_signature( $issue ) {
        ...
        $norm = trim( preg_replace( self::$punct_pattern, '', $norm ) );
       
        // Identifier/SKU/GTIN issues - all variations normalize to same signature
        // Includes Google API format: "limited performance due to missing value [gtin]"
        if ( strpos( $norm, 'missing sku' ) !== false || 
             ...
             strpos( $norm, '[gtin]' ) !== false ||
             strpos( $norm, 'missing value gtin' ) !== false ) {
        ...
        if ( strpos( $norm, 'product pause' ) !== false || strpos( $norm, 'pause expired' ) !== false ) {
            return 'product_pause_expired';
        }
    ...
    public static function run_gmc_scan_logic( $batch_size = 100, $paged = 1 ) {
        ...
        // ALWAYS log the start of the scan
        $debug_log[] = '=== GMC Scan Started ===';
        $debug_log[] = 'Batch size: ' . $batch_size . ', Page: ' . $paged;
        $debug_log[] = 'Is Pro: ' . ( class_exists( 'Cirrusly_Commerce_Core' ) && Cirrusly_Commerce_Core::cirrusly_is_pro() ? 'YES' : 'NO' );
        ...
        if ( class_exists( 'Cirrusly_Commerce_Core' ) && 
            ...
            $debug_log[] = 'Calling fetch_google_real_statuses()...';
            $google_issues = Cirrusly_Commerce_GMC_Pro::fetch_google_real_statuses();
            
            // Debug: Log what we got from Google
            if ( is_array( $google_issues ) && ! empty( $google_issues ) ) {
                $debug_log[] = '✓ Fetched Google issues for ' . count( $google_issues ) . ' products';
                foreach ( $google_issues as $pid => $issues ) {
                    foreach ( $issues as $issue ) {
                        $debug_log[] = sprintf( '  - PID %d: "%s" (type: %s)', $pid, $issue['msg'] ?? 'NO MSG', $issue['type'] ?? 'NO TYPE' );
                    }
                }
            } else {
                $debug_log[] = '⚠ No Google issues fetched (empty or not array). Type: ' . gettype( $google_issues );
                $google_issues = array();
            }
        } else {
            $debug_log[] = '⚠ Pro mode not active or GMC_Pro class not available';
        }
        ...
        $debug_log[] = 'Found ' . count( $products ) . ' products to scan';
        ...
        foreach ( $products as $pid ) {
            ...
            if ( 'no' !== $is_custom && ! $p->get_sku() ) {
                $product_issues[] = array(
                    'type' => 'warning',
                    'msg'  => 'Missing GTIN (Identifier)',
                    'reason' => 'Ensure that this product does not have a UPC/GTIN.'
                );
            }
            ...
            foreach ( $monitored_terms as $category => $terms ) {
                foreach ( $terms as $word => $rule ) {
                    ...
                    // Case-sensitive terms (like WHO) should ONLY match exact case
                    // Default is case-insensitive for most terms
                    $is_case_sensitive = isset( $rule['case_sensitive'] ) && $rule['case_sensitive'];
                    
                    // For case-sensitive: match exact case only (no 'i' flag)
                    // For case-insensitive: match any case ('i' flag)
            ...
            foreach ( $product_issues as $local_issue ) {
                $sig = self::get_issue_signature( $local_issue );
                $local_signatures[] = $sig;
                $norm_msg = strtolower( preg_replace( self::$punct_pattern, '', $local_issue['msg'] ) );
                $debug_log[] = sprintf( 'Local issue PID %d: "%s"', $pid, $local_issue['msg'] );
                $debug_log[] = sprintf( '  Normalized: "%s" -> sig: %s', $norm_msg, $sig );
            }

            if ( isset( $google_issues[ $pid ] ) ) {
                foreach ( $google_issues[ $pid ] as $g_issue ) {
                    $g_sig = self::get_issue_signature( $g_issue );
                    $norm_google = strtolower( preg_replace( self::$punct_pattern, '', $g_issue['msg'] ) );
                    
                    $debug_log[] = sprintf( 'Google issue PID %d: "%s"', $pid, $g_issue['msg'] );
                    $debug_log[] = sprintf( '  Normalized: "%s" -> sig: %s', $norm_google, $g_sig );
                    $debug_log[] = '  Local sigs: [' . implode( ', ', $local_signatures ) . ']';
                    
                    if ( ! in_array( $g_sig, $local_signatures, true ) ) {
                        $product_issues[] = $g_issue;
                        $debug_log[] = '  ✓ ADDED (no match)';
                    } else {
                        $debug_log[] = '  ✗ SKIPPED (duplicate match)';
                    }
                }
            }
        ...
        // CRITICAL FIX: Process products with Google-only issues that weren't in the scanned batch
        // This ensures image quality issues and other Google-only findings don't get lost
        foreach ( $google_issues as $g_pid => $g_issues ) {
            // Skip if we already processed this product in the main loop
            if ( in_array( $g_pid, $products, true ) ) {
                continue;
            }
            
            // Verify product exists before adding
            $g_product = wc_get_product( $g_pid );
            if ( ! $g_product ) {
                $debug_log[] = sprintf( 'Skipping Google-only PID %d (product not found)', $g_pid );
                continue;
            }
            
            // Add Google-only issues (no local scan was performed for this product)
            $debug_log[] = sprintf( 'Processing Google-only issues for PID %d (not in batch)', $g_pid );
        ...
        // Store debug logs in transient for display (ALWAYS, even if empty)
        $debug_log[] = '=== Scan Complete ===';
        $debug_log[] = 'Results: ' . count( $results ) . ' products with issues';
        set_transient( 'cirrusly_gmc_scan_debug', $debug_log, HOUR_IN_SECONDS );
