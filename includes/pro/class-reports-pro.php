...
class Cirrusly_Commerce_Reports_Pro {
    ...
    public static function generate_and_send( $config ) {
        ...
        // 2. Ensure Mailer is loaded
        if ( ! class_exists( 'Cirrusly_Commerce_Mailer' ) ) {
            require_once CIRRUSLY_COMMERCE_PATH . 'class-mailer.php';
        }

        // 3. Send using template
        $to = Cirrusly_Commerce_Mailer::get_recipient(
            'weekly_profit',
            !empty( $config['email_recipient'] ) ? $config['email_recipient'] : ''
        );
        
        // Check if email type is enabled
        if ( ! Cirrusly_Commerce_Mailer::is_email_enabled( 'weekly_profit' ) ) {
            return;
        }
